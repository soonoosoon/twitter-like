...
const makeLike = async () => {
  if (props.enableLiked) {
    await userStore.cancelLikeInTweet(props.id)
    await userStore.cancelLikeInUser(props.id)
  } else {
    await userStore.confirmLikeInTweet(props.id)
    await userStore.confirmLikeInUser(props.id)
  }
}
</script>
...
<template>
  <div
    class="grid grid-cols-[auto,1fr] gap-3 px-3 pt-3 pb-1 border-b hover:bg-neutral-100 hover:transition-colors duration-500 ease-out"
  >
    ...
    <div>
      ...
      <div class="-ml-2 flex text-xs text-slate-600 hover:transition-colors duration-500 ease-out">
        ...
        <div class="flex items-center gap-1 group mr-auto" @click="makeLike">
          ...
          />
          <span v-show="like" :class="enableLiked ? 'text-rose-500' : ''" class="group-hover:text-rose-500">{{
            like
          }}</span>
        </div>
